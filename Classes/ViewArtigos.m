...
- (void)viewDidLoad {
	...
	[self.view addSubview:[functions headerTableview:@"Artígos"]];
	
	artigo = [Dados sharedManager];
	
	arrayArtigos = [[NSMutableArray alloc] init];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    int line = 0;
	for (int i = 0; i < [artigo.arguments count]; i++) {
		artigos = [artigo.arguments objectAtIndex:i];
		if (![artigos.channelAuthor isEqualToString:@""]) {
			line += 1;
			[arrayArtigos addObject:artigos];
		}
	}
    return line;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	News * artigosArray = [arrayArtigos objectAtIndex:indexPath.row];
	// Configure the cell...
	cell.imageView.image  = [UIImage imageNamed:@"listArtigos.png"];
	cell.textLabel.text = artigosArray.channelEditoria;
	cell.detailTextLabel.text = artigosArray.channelAuthor;
	cell.detailTextLabel.font = [UIFont fontWithName:@"TrebuchetMS" size:11];
	cell.textLabel.font = [UIFont fontWithName:@"TrebuchetMS" size:13];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	[detailViewController setTitulo:@"Artígos"];
	[detailViewController set_news:[arrayArtigos objectAtIndex:indexPath.row]];
